...
% 
    ...
    % The original code for this seems very complicated.  Fuck all that.
    
    % Parse args
    % desc_ch is a cell array of strings.  But in all the use cases I've seen, the
    % array is of length one.  And the string is either '0' or '1'.
    if isempty(desc_ch) ,
        error('readDesc() needs a channel index') ;
    end
    channel_index_as_string = desc_ch{1} ;
    channel_index = str2double(channel_index_as_string) ;
